...
void Ieee80211Mesh::initialize(int stage)
{
    ...
    // Stage 3 used for configurate the GateWay Data
    if (stage==3)
    {
        if (par("IsGateWay"))
        {
...
{
    /* Extraer la etiqueta y la direcciï¿½n de enlace del siguiente salto */
...
void Ieee80211Mesh::mplsDataProcess(LWMPLSPacket * mpls_pk_ptr,MACAddress sta_addr)
{
    ...
    if (testMplsData)
    {
        ...
        if (!label_found)
        {
            ...
            // ï¿½ es necesario destruir label_msg_ptr? mirar la memoria
...
//       small random delay. Avoid the collision
        ...
        scheduleAt(simTime()+par("BroadcastDelay"),meshFrame);
        break;
    }
}
...
{
// ï¿½Como? las colas estan en otra parte.
    bool purge;
...
bool Ieee80211Mesh::macLabelBasedSend (Ieee80211DataFrame *frame)
{
    ...
    if (isGateWay)
    {
        ...
        if (frame->getAddress4()!=myAddress)
        {
...
void Ieee80211Mesh::sendOrEnqueue(cPacket *frame)
{
    ...
    if (isGateWay && frameAux)
    {
        ...
        if (frameAux->getReceiverAddress().isBroadcast())
        {
            ...
            {
                sendDirect(frameAux->dup(),it->second.gate);
            }
        }
        else
        {
